...
### seaborn `multiple` 파라미터 정리

> `multiple`은 `seaborn.hisplot()` 또는 `displot()`에서 `hue`로 나뉜 여러 그룹(범주)를 어떻게 시각적으로 배치할지 설정하는 옵션

| 값 (`multiple=...`) | 의미                                 | 설명 |
|---------------------|--------------------------------------|------|
| `'layer'` (기본값)   | 겹치기 (투명도 활용)                  | 그룹별 막대를 겹쳐서 그림 |
| `'dodge'`           | 옆으로 나란히                        | 그룹별 막대를 수평 분리 |
| `'stack'`           | 위로 쌓기 (누적)                     | 값들을 더해서 누적 막대 형태로 표현 |
| `'fill'`            | 비율 누적                            | 100% 기준 비율 누적 그래프로 표시 |
